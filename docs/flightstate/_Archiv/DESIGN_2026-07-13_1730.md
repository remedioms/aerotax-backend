...
| T4b | dep off-block signal **and** off-block older than `TAXI_OUT_MAX_S` (25 min) **and** `now` < expected arrival **and** **no** live position at all | `AIRBORNE` (conf=**estimated**, `phase_source=offblock_time`, **live=None** — time-evidence, never an invented dot) |
...
**T4b — off-block time-elevation (owner 2026-07-13, ONE truth):** a flight that pushed back far longer than a plausible taxi window, is still before its expected arrival, and has **no live position we can see** (over ocean / no ADS-B) is airborne even without a fix — otherwise an 11h long-haul off-block for 3 h would stay "TAXI_OUT" forever on every surface. It is elevated to `AIRBORNE` with `phase_conf=estimated` and `live=None` (time-evidence, **never** a guessed dot). Bounded by expected arrival (`sched_arr` + observed `arr_delay`, or a revised `est_arr`) so it can never outlive the flight; a real landing / monotonicity handles the terminal end. **If there IS a live position it must FAIL the gate to reach T4** (visibly on the ground) → then T4b is skipped and it stays `TAXI_OUT` (the plane really is still taxiing — the ghost-fix). This is the single boundary all projections share; crew_state no longer carries its own 25-min taxi cap.
