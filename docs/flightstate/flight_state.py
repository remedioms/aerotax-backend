#!/usr/bin/env python3
"""
AeroX Unified FlightState Engine — reference prototype (pure reducer).
...
def _apply_monotonicity(cur, prior, now):
    """Once terminal (LANDED/ARRIVED), no regression to an earlier lifecycle phase
    except on a FRESH HARD signal newer than the terminal one."""
    if not prior:
        return cur
    if prior.get("phase") in (LANDED, ARRIVED) and cur["phase"] in _LIFECYCLE_RANK:
        if _LIFECYCLE_RANK[cur["phase"]] < _LIFECYCLE_RANK.get(prior["phase"], 99):
...
def _show(fs, expect_phase, expect_live_none=None, extra_check=None):
    print(f"  phase      : {fs['phase']}  ({fs['phase_conf']}, via {fs['phase_source']})")
    print(f"  in_flight  : {fs['in_flight']}   live: "
          f"{'null' if fs['live'] is None else fs['live']['source'] + '/' + fs['live']['conf']}")
    print(f"  route      : {fs['route']['dep']}->{fs['route']['dst']} "
          f"({fs['route']['conf']}, {fs['route']['source']})")
    print(f"  reg        : {fs['reg']} ({fs['reg_conf']}) swap={fs['reg_swap']}")
    print(f"  eta        : {fs['times']['eta_iso']} ({fs['times']['eta_conf']})")
    print(f"  delay      : known={fs['delay']['known']} min={fs['delay']['min']} "
          f"on_time={fs['on_time']}")
    ok = True
    if fs["phase"] != expect_phase:
        ok = False; print(f"  !! EXPECTED phase {expect_phase}")
    if expect_live_none is True and fs["live"] is not None:
        ok = False; print("  !! EXPECTED live=None")
    if expect_live_none is False and fs["live"] is None:
        ok = False; print("  !! EXPECTED live populated")
    if extra_check:
        msg = extra_check(fs)
        if msg:
            ok = False; print(f"  !! {msg}")
    print(f"  => {'OK' if ok else 'FAIL'}")
    return ok


def main():
    NOW = 1_783_584_000  # fixed clock for determinism (2026-07-09T04:00:00Z)
    FRA = (50.03, 8.57); GVA = (46.24, 6.11); HND = (35.55, 139.78)
    SIBERIA = (61.7, 90.2); MANNHEIM = (49.48, 8.52)
    results = []

    # ---- Scenario 1: D-AINV FRA->GVA taxi (the ghost bug) ----
    _hdr("Scenario 1  D-AINV FRA->GVA  taxi/pushback  (must NOT show cruise ghost)")
    s1 = resolve_flight_state(
        ...
        observations=[
            ...
            Observation("reg", {"reg": "D-AINV", "ac_type": "A321"}, "aircraft_live",
                        NOW - 30, 0.8),
            # board dep-side "Abgeflogen" = OFF-BLOCK, not proven airborne
            Observation("phase_hard", TAXI_OUT, "board", NOW - 120, 0.9, meta={"side": "dep"}),
            # aircraft_live snapshot near FRA: gs=15, alt=None, on_ground_raw=false
    ...
    results.append(_show(s1, TAXI_OUT, expect_live_none=True,
                         extra_check=lambda fs: None if fs["times"]["eta_conf"] != SIMULATED
                         else "ETA must NOT be gs-extrapolated for a taxiing plane"))

    # ---- Scenario 2: LH716 FRA->HND over Siberia (ADS-B blind, snapshot fills) ----
    _hdr("Scenario 2  LH716 FRA->HND  over Siberia  (ADS-B blind -> aircraft_live)")
    s2 = resolve_flight_state(
        ...
        observations=[
            ...
            # ADS-B collector ran but structurally blind over Siberia -> ABSENT
            Observation("position", None, "adsb", NOW, status="absent"),
            # aircraft_live southern-route snapshot: real cruise fix
            Observation("position", {"lat": SIBERIA[0], "lon": SIBERIA[1], "track": 61,
                                     "gs_kt": 476, "alt_ft": 37000, "on_ground_raw": False,
                                     "position_source": 3}, "aircraft_live", NOW - 90, 0.8),
        ], now=NOW)
    results.append(_show(s2, AIRBORNE, expect_live_none=False,
                         extra_check=lambda fs: None if fs["live"]["source"] == "aircraft_live"
                         else "position must come from aircraft_live snapshot"))

    # ---- Scenario 3: aircraft swap + codeshare (LH marketing / LX operating) ----
    _hdr("Scenario 3  Codeshare + aircraft swap  (roster tail corrected)")
    s3 = resolve_flight_state(
        keys={"flight": "LH2557", "date": "2026-07-09", "dep_iata": "ZRH",
              "arr_iata": "FRA", "dep_ll": (47.46, 8.55), "arr_ll": FRA,
              "roster_tail": "HB-OLD", "sched_dep": "2026-07-09T10:00:00Z",
              "sched_arr": "2026-07-09T11:05:00Z", "sched_dep_ts": NOW - 1200},
        observations=[
            # codeshare collector: LH2557 marketed, operated as LX1071 (HB-JCA)
            Observation("reg", {"reg": "HB-JCA", "ac_type": "BCS3"}, "board",
                        NOW - 60, 0.98, meta={"flightno_matched": True,
                                              "codeshare": {"oper_flight": "LX1071",
                                                            "oper_callsign": "SWR1071",
                                                            "oper_carrier": "LX"}}),
            ...
            Observation("position", {"lat": 48.5, "lon": 8.4, "track": 20, "gs_kt": 420,
                                     "alt_ft": 33000, "position_source": 0}, "adsb",
                        NOW - 40, 0.95),
            Observation("delay", {"delay_known": True, "arr_delay_min": 8, "dep_delay_min": 6},
                        "board", NOW - 60, 0.9),
        ], now=NOW)
    results.append(_show(s3, AIRBORNE, expect_live_none=False,
                         extra_check=lambda fs: None if (fs["reg"] == "HB-JCA"
                         and fs["reg_swap"] and fs["keys"]["flight"] == "LX1071")
                         else "swap tail HB-JCA + operating flight LX1071 expected"))

    # ---- Scenario 4: clean cruising flight (fresh ADS-B) ----
    _hdr("Scenario 4  Clean cruise  (fresh ADS-B, board airborne)")
    s4 = resolve_flight_state(
        ...
        observations=[
            ...
            Observation("position", {"lat": 47.9, "lon": 7.4, "track": 205, "gs_kt": 431,
                                     "alt_ft": 34000, "position_source": 0}, "adsb",
                        NOW - 17, 0.95),
            Observation("delay", {"delay_known": True, "arr_delay_min": 3, "dep_delay_min": 5},
                        "board", NOW - 60, 0.9),
        ], now=NOW)
    results.append(_show(s4, AIRBORNE, expect_live_none=False,
                         extra_check=lambda fs: None if (fs["live"]["source"] == "adsb"
                         and fs["on_time"] is True) else "expected ADS-B fix + on_time"))

    # ---- Scenario 5: cancelled ----
    _hdr("Scenario 5  Cancelled  (board hard cancel wins, no dot)")
    s5 = resolve_flight_state(
        keys={"flight": "LH444", "date": "2026-07-09", "dep_iata": "MUC",
              "arr_iata": "JFK", "dep_ll": (48.35, 11.78), "arr_ll": (40.64, -73.78),
              "roster_tail": "D-AIMA", "sched_dep": "2026-07-09T12:00:00Z",
              "sched_arr": "2026-07-09T21:00:00Z", "sched_dep_ts": NOW - 600},
        observations=[
    ...
    results.append(_show(s5, CANCELLED, expect_live_none=True))

    # ---- Scenario 6: stale board 'landed' must NOT un-fly a fresh airborne fix ----
    _hdr("Scenario 6  Monotonicity  (prior LANDED, stale) -> fresh hard airborne re-flies")
    prior = {"phase": LANDED, "conf": OBSERVED, "obs_ts": NOW - 4000,
             "sticky_airborne": False}
    s6 = resolve_flight_state(
        keys={"flight": "LH880", "date": "2026-07-09", "dep_iata": "FRA",
              "arr_iata": "LHR", "dep_ll": FRA, "arr_ll": (51.47, -0.45),
              "roster_tail": "D-AITA", "sched_dep": "2026-07-09T12:00:00Z",
              "sched_arr": "2026-07-09T12:55:00Z", "sched_dep_ts": NOW - 1800},
        observations=[
            Observation("event", {"event": "takeoff", "airport": "EDDF"},
                        "warehouse_event", NOW - 200, 0.95),  # FRESH hard, newer than prior
            Observation("position", {"lat": 50.5, "lon": 5.0, "track": 280, "gs_kt": 440,
                                     "alt_ft": 35000, "position_source": 0}, "adsb",
                        NOW - 20, 0.95),
            Observation("route", {"dep": "FRA", "dst": "LHR", "confidence": "confirmed"},
                        "fr24_bulk", NOW - 30, 0.9),
        ], now=NOW, prior=prior)
    results.append(_show(s6, AIRBORNE, expect_live_none=False))

    _hdr("PROJECTIONS (Scenario 4 across all four surfaces)")
    print("  my_flight_status:", json.dumps(project_my_flight_status(s4)))
    print("  flight_live     :", json.dumps({k: project_flight_live(s4)[k]
          for k in ("phase", "in_flight", "eta_iso", "eta_conf", "arr_delay_min")}))
    print("  crew_status     :", json.dumps(project_crew_status(s4)))
    print("  friend_leg      :", json.dumps({k: project_friend_leg(s4)[k]
          for k in ("status", "delay_known", "arr_delay_min", "live")}))

    _hdr(f"RESULT: {sum(results)}/{len(results)} scenarios OK")
    if all(results):
        print("  ALL GREEN")
        return 0
