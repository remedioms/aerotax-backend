#!/bin/bash
# AeroX Signup-Tages-Digest (Host-Wrapper) — ersetzt die Echtzeit-Signup-Mails.
# Cron: 30 5 * * * /opt/aerox/signup_digest_wrapper.sh   (07:30 DE-Sommerzeit)
# Report-Zeile 1 = Subject, Rest = Body. Mailt via Resend an SIGNUP_NOTIFY_EMAIL.
# Exit-Code immer 0 (kein Cron-Spam).
set -uo pipefail

LOG=/tmp/aerox-signup-digest-out.txt
...
REPORT=$(docker exec -i aerotax-backend python3 - < /opt/aerox/signup_digest_report.py 2>>"$LOG")
if [ -z "$REPORT" ]; then
    echo "$(date -u +%FT%TZ) leerer Digest — siehe Fehler oben" >> "$LOG"
    exit 0
fi
